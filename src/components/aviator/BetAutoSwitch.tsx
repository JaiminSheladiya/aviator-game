...
const BetAutoButton = ({ disabled, title, onClick }: { disabled: boolean, title: string, onClick: () => void }) => {
    return (
        <button disabled={disabled} onClick={onClick} className='flex justify-center items-center w-[48px] lg:w-[70px] h-[18px] lg:h-[26px] rounded-full text-[8px] lg:text-lg z-10 disabled:opacity-80 disabled:cursor-not-allowed' style={{fontSize : "0.8rem"}}>
            {title}
        </button>
    )
}
