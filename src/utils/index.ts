...
export const smoothen = (t: number, dimension: dimensionType) =>
  Math.sin((Math.PI * t) / (2 * dimension.width)) * dimension.width;

export const _drawBar =
  (width: number, color: ColorSource) => (g: Graphics) => {
    g.clear();
    g.beginFill(color, 1);
...
export function interpolate(
  ...
  return Math.max(
    Math.min(y1, y2),
    Math.min(Math.max(y1, y2), ((y2 - y1) * (x - x1)) / (x2 - x1) + y1)
  );
}
...
export const getUTCTimefromUTCTime: (timeString: string) => Date = (
  timeString: string
) => {
